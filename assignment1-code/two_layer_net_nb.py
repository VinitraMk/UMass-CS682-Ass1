...
Original file is located at
    https://colab.research.google.com/drive/11cICXhzMTGiXS0k9A4LOdGIlIeQv44u9
...
hidden_size = 64
net = TwoLayerNet(input_size, hidden_size, num_classes)
learning_rates = [1e-4, 2e-4, 3e-4, 1e-3, 2e-3, 3e-3, 4e-3, 5e-3]
regs = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2]
combos = [(v[0], v[1]) for v in list(itertools.product(learning_rates, regs))]
best_acc = -1
best_combo = ()
stats = {}
...
for combo in combos:
    ...
    stats = net.train(X_train, y_train, X_val, y_val,
        ...
        learning_rate=combo[0], learning_rate_decay=0.8,
 ...
 *Your explanation:*
 1. True - The gap between the training and test accuracy indicates that the model has overfit the data.
 Getting more training data means getting a better reprensentation of the test data. It will mean
 having more data for cross-validation, improving hyperparameter testing.
 All this will help the model generalize it better, thereby increasing test accuracy.

 2. False - Adding mor hidden units will cause the model to overfit, which is already the case.
 Adding more hidden unit will either have no effect or will make the the performance worse.

 3. True - Increasing the regularization strength decreases the dependency on initial weights.
 This way overfitting can be prevented and a simpler function can be found that generalizes better.
